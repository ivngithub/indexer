...
if __name__ == '__main__':
    ...
    while indexer.end_block != indexer.last_block:
        ...
        try:
            filter_ = exchange_contract.events.Fill.createFilter(
        ...
        for i, event in enumerate(events):
            ...
            if previous_event.transactionHash.hex() == event.transactionHash.hex():

                seller, _ = Account.objects.get_or_create(address=event['args']['makerAddress'])
                buyer, _ = Account.objects.get_or_create(address=previous_event['args']['makerAddress'])
                trade = Trade(
                    seller=seller,
                    buyer=buyer,
