...
if __name__ == '__main__':
    ...
    while indexer.end_block != indexer.last_block:
        ...
        try:
            filter_ = exchange_contract.events.PunkBought.createFilter(
        ...
        for event in events:
            seller, _ = Account.objects.get_or_create(address=event['args']['fromAddress'])
            buyer, _ = Account.objects.get_or_create(address=event['args']['toAddress'])
            trade = Trade(
                seller=seller,
                buyer=buyer,
